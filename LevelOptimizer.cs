...
public class LevelOptimizer
{
    ...
    const float MUTATION_RATE = 0.3f;//The rate at which to mutate population members
    const int NUM_CROSSOVERS = 20;//The number of crossovers/children to produce
    ...
    public Level GetLevel(Evaluator evaluator, float goalValue)
    {
        ...
        while (bestFitnessSoFar < threshold && attempts < MAX_ATTEMPTS)
        {
            ...
            // use sample two parents
            Level Parent1 = Sample(population, evaluator, threshold);
            Level Parent2 = Sample(population, evaluator, threshold);
            while (Parent1 == Parent2)
            {
                Parent2 = Sample(population, evaluator, threshold);
            }
            Level child = Crossover(Parent1, Parent2);
