...
public class NavMeshHandler : NodeHandler
{
    ...
    public override void CreateNodes()
    {
            ...
            foreach (Vector2 b in allPoints)
            {
                foreach (Vector2 c in allPoints)
                {
                    ...
                    {   
                        ...
                        Polygon triangle = new Polygon(new Vector2[] { a, b, c });
                        if (!meshes.Contains(triangle)){
                            foreach(Polygon obstacle in (ObstacleHandler.Instance.Obstacles).Concat(meshes).ToList()){
                                if (obstacle.AnyIntersectWithoutSharedPoint(a,b) 
                                || obstacle.AnyIntersectWithoutSharedPoint(a,c)
                                || obstacle.AnyIntersectWithoutSharedPoint(b,c)
                                || (!obstacle.HasLine(new Vector2[] {a,b}) && obstacle.HasPoint(a) && obstacle.HasPoint(b))
                                || (!obstacle.HasLine(new Vector2[] {a,c}) && obstacle.HasPoint(a) && obstacle.HasPoint(c))
                                || (!obstacle.HasLine(new Vector2[] {b,c}) && obstacle.HasPoint(b) && obstacle.HasPoint(c))) {
                                    valid = false;
                                    break;
                                }
                            }
        ...
        Debug.Log("total amount= " + meshes.Count().ToString());
